...
class Custom_WC_Widget_Product_Categories extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'A list or dropdown of product categories.', 'alaska' );
		$this->widget_id          = 'woocommerce_product_categories';
		$this->widget_name        = __( 'WooCommerce Product Categories', 'alaska' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Product Categories', 'alaska' ),
			...
			'orderby' => array(
				...
				'options' => array(
					'order' => __( 'Category Order', 'alaska' ),
					'name'  => __( 'Name', 'alaska' )
				)
			),
			'dropdown' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Show as dropdown', 'alaska' )
			),
			'count' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Show post counts', 'alaska' )
			),
			'hierarchical' => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => __( 'Show hierarchy', 'alaska' )
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Only show children of the current category', 'alaska' )
			)
		);
		parent::__construct();
	}
	...
	public function widget( $args, $instance ) {
		...
		// List
			...
			$list_args['show_option_none']           = __('No product categories exist.', 'alaska' );
