...
class Custom_WC_Widget_Layered_Nav extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'alaska' );
		$this->widget_id          = 'woocommerce_layered_nav';
		$this->widget_name        = __( 'WooCommerce Layered Nav', 'alaska' );
	...
	public function init_settings() {
		...
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Filter by', 'alaska' ),
				'label' => __( 'Title', 'alaska' )
			),
			'attribute' => array(
				'type'  => 'select',
				'std'   => '',
				'label' => __( 'Attribute', 'alaska' ),
				'options' => $attribute_array
			),
			'display_type' => array(
				'type'  => 'select',
				'std'   => 'list',
				'label' => __( 'Display type', 'alaska' ),
				'options' => array(
					'list'      => __( 'List', 'alaska' ),
					'dropdown'  => __( 'Dropdown', 'alaska' )
				)
			),
			'query_type' => array(
				'type'  => 'select',
				'std'   => 'and',
				'label' => __( 'Query type', 'alaska' ),
				'options' => array(
					'and' => __( 'AND', 'alaska' ),
					'or'  => __( 'OR', 'alaska' )
				)
		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( __( 'Any %s', 'alaska' ), wc_attribute_label( $taxonomy ) ) .'</option>';
