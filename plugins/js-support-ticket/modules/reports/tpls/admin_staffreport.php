...
<script type="text/javascript">
    function updateuserlist(pagenum){
        jQuery.post(ajaxurl, {action: 'jsticket_ajax', jstmod: 'staff', task: 'getusersearchstaffreportajax',userlimit:pagenum}, function (data) {
            if(data){         
    ...
    jQuery(document).ready(function ($) {
        ...
        jQuery("a#userpopup").click(function (e) {
            ...
            jQuery.post(ajaxurl, {action: 'jsticket_ajax', jstmod: 'staff', task: 'getusersearchstaffreportajax'}, function (data) {
                if(data){                    
                    jQuery("div#records").html("");
                    jQuery("div#records").html(data);
                    setUserLink();
        ...
        jQuery("form#userpopupsearch").submit(function (e) {
            ...
            jQuery.post(ajaxurl, {action: 'jsticket_ajax', name: name, emailaddress: emailaddress, jstmod: 'staff', task: 'getusersearchstaffreportajax'}, function (data) {
                if (data) {
                    jQuery("div#records").html(data);
                    setUserLink();
                }
...
?>
<span class="js-adminhead-title"> <a class="jsanchor-backlink" href="<?php echo admin_url('admin.php?page=reports');?>"><img src="<?php echo jssupportticket::$_pluginpath; ?>includes/images/back-icon.png" /></a> <span class="jsheadtext"><?php echo __("Report By Staff Members", 'js-support-ticket') ?></span>
...
if(!empty(jssupportticket::$_data['staffs_report'])){
	foreach(jssupportticket::$_data['staffs_report'] AS $staff){ ?>		
		<div class="js-admin-staff-wrapper">
			<a href="<?php echo admin_url('admin.php?page=reports&jstlay=staffdetailreport&id='.$staff->id.'&date_start='.jssupportticket::$_data['filter']['date_start'].'&date_end='.jssupportticket::$_data['filter']['date_end']); ?>" class="js-admin-staff-anchor-wrapper">
