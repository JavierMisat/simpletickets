...
class JSSTticketModel {
    ...
    {
        $ticketid = new TicketPQR();

        return $ticketid->getIdSolicitud();
    }
...
class TicketPQR extends JSSTticketModel
{
    private static $codigoCliente = 3979;
    private static $consecutivo = 1;
    private static $codigoAnio;

    public function __construct()
    {
        static::$consecutivo = $this->getConsecutivoActual();
        static::$codigoAnio = $this->getFechaConsecutivoActual();
    }

    public function getIdSolicitud()
    {
        return static::$codigoCliente . static::$codigoAnio . static::$consecutivo;
    }

    private function getConsecutivoActual()
    {
        if ($this->checkFechaHabil()) {
            return $this->addCeros(static::$consecutivo);
        } else {
            if (is_numeric($this->getMaximo())) {
                return $this->addCeros($this->getNumeroConsecutivoAnterior() + 1);
            }
        }
        return $this->addCeros(static::$consecutivo);
    }

    private function checkFechaHabil()
    {
        //Setear zona horaria de colombia
        date_default_timezone_set('America/Bogota');

        // Año actual y año de la ultima solicitud registrada
        $anioActual = date('y');
        $anioActualRegistrado = $this->getFechaConsecutivoMaximoActual();

        //Datos de fecha actual
        $diaActual = date('Y-m-d');
        $diaHabilAnio = date('Y-m-d', strtotime($diaActual . ' +1 Weekday'));

        if ($anioActualRegistrado != $anioActual) {
            return static::$consecutivo;

        }
    }

    private function getDiaLaboral($diaActual)
    {
        $workingDays = [1, 2, 3, 4, 5]; # date format = N (1 = Monday, ...)
        $holidayDays = ['*-12-25', '*-01-01', '*-12-24', '*-12-25']; # variable and fixed holidays

        list($anio, $mes, $dia) = explode('-', $diaActual);
        $dia--;

        $diaActual = new DateTime("$anio-$mes-$dia");
        $dates = [];
        $dates[] = $diaActual->format('Y-m-d');
        $days = 1;
        while ($days) {
            $diaActual->modify('+1 day');

            if (!in_array($diaActual->format('N'), $workingDays)) {
                continue;
            }
            if (in_array($diaActual->format('Y-m-d'), $holidayDays)) {
                continue;
            }
            if (in_array($diaActual->format('*-m-d'), $holidayDays)) {
                continue;
            }
    ...
    private function getFechaConsecutivoActual()
    {
        return date('y');
    }

    private function getNumeroConsecutivoAnterior()
    {
        return $this->addCeros(substr($this->getMaximo(), 6, 8));
    }

    private function getFechaConsecutivoMaximoActual()
    {
        return substr($this->getMaximo(), 4, -8);
    }
    ...
    private function addCeros($number)
    {
    ...
    {
       // return new PDO('mysql:host=localhost;dbname=pqr_simple', "root", "");
    }
